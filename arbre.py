...
class TexturedTree(Node):
    def __init__(self, shader, position, leavesTextures, trunkTextures, shadow_map_tex):
        super().__init__()
        random.seed()
...
class Treemapping(Node): # plus tard, mapper la height de détéction ou non des arbres valides sur celle de l'eau +5 ( la plage en gros )
    def __init__(self, shader, position_Array, leavesTextures_path1, leavesTextures_path2, trunkTextures_path, nb_gen, shadow_map_tex):
        super().__init__()
        cpt =0        
        random.seed()
        ...
        while cpt < nb_gen: # generation basique aléatoire, très couteux, GPU instancing using bufferisation (explication prof) plus tard si possible en temps.
            ...
            if plant_coordinates[1]<= WATER_LEVEL : #en dessous du niveau de l'eau
                ...
                if choix == 0 :
                    # ajout d'un pin 
                   self.add(TexturedPineTree(shader, position_Array[plant],leavesTextures1,trunkTextures, shadow_map_tex))
                   pass
                elif choix ==1 :
                    #ajout d'un arbre basique
                    self.add(TexturedTree(shader, position_Array[plant],leavesTextures2 ,trunkTextures, shadow_map_tex))
                else : 
